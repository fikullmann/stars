...
data class MUnaryPred<T : EntityType<*, *, *, *, *>>(val ref: Ref<T>, val phi: () -> Boolean) : MState
...
class Buf2(val lhs: MutableList<Pdt<Proof>> = mutableListOf(), val rhs: MutableList<Pdt<Proof>> = mutableListOf()) {
    fun add(p1s: List<Pdt<Proof>>, p2s: List<Pdt<Proof>>) {
        lhs.addAll(p1s)
        rhs.addAll(p2s)
    }

    fun take(ref: MutableList<Ref<EntityType<*, *, *, *, *>>>, f: (Proof, Proof) -> Proof): List<Pdt<Proof>> {
...
class BufPrevNext(val inner: MutableList<Pdt<Proof>> = mutableListOf(), val tss: MutableList<TS> = mutableListOf()) {
    fun clearInner() {
        inner.clear()
    }
    ...
    fun take(ref: MutableList<Ref<EntityType<*, *, *, *, *>>>, prev: Boolean, interval: Interval): List<Pdt<Proof>> {
        val result = mutableListOf<Pdt<Proof>>()
...
class BufT(val inner: MutableList<Pdt<Proof>> = mutableListOf(), val tsTp: MutableList<Pair<TS, TP>> = mutableListOf()) :
...
class Buf2T(val lhs: MutableList<Pdt<Proof>> = mutableListOf(), val rhs: MutableList<Pdt<Proof>> = mutableListOf(), val tsTp: MutableList<Pair<TS, TP>> = mutableListOf()) :
    Iterable<Triple<Pair<Pdt<Proof>, Pdt<Proof>>, TS, TP>> {
    fun add(p1s: List<Pdt<Proof>>, p2s: List<Pdt<Proof>>, ts: Pair<TS, TP>) {
        lhs.addAll(p1s)
        rhs.addAll(p2s)
        tsTp.add(ts)
    }
    ...
    override fun iterator(): Iterator<Triple<Pair<Pdt<Proof>, Pdt<Proof>>, TS, TP>> = Buf2TIterator(this)

}

class Buf2TIterator(val self: Buf2T) : Iterator<Triple<Pair<Pdt<Proof>, Pdt<Proof>>, TS, TP>> {
    override fun hasNext(): Boolean = self.lhs.isNotEmpty() && self.rhs.isNotEmpty() && self.tsTp.isNotEmpty()
