...
data class MinAux(
    ...
    fun copy() = MinAux(sAlphas.map { it.copy() }.toMutableList(),
     ...
     */
    fun updateMinAux(interval: Interval, nts: TS, ntp: TP, factor: Double, p1: Proof): MutableList<Proof> {
     ...
     * updates the Eaux structure at arrival of a new TS/TP
     */
    fun update(interval: Interval, nts: TS, ntp: TP, factor: Double, p1: Proof): Pair<MutableList<Proof>, MinAux> {
        val copy = copy()
    ...
    private fun shiftMinAux(interval: BoundedInterval, nts: TS, ntp: TP, factor: Double, end: Double? = null) {
        ...
        tsTps.forEach { (ts, tp) ->
            ...
            if (correct >= factor) {
                ...
                optimalProofs.add(ts to SatPastMinPrev(tp, ltp, correct, sAlphas.map { it.second }.toMutableList()))
            } else {
                optimalProofs.add(ts to VPastMinPrev(tp, vAlphas.last().second.at(), correct, vAlphas.map { it.second }.toMutableList()))
            }
