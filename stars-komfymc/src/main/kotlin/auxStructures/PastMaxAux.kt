...
class PastMaxAux(
    ...
    private fun evalPMaxAux(tp: TP, factor: Double): Proof {
        ...
        return if (correct < factor) {
            ...
            VPastMaxPrev(tp, vAlphasIn.first().second.at(), correct, vAlphasIn.map { it.second }.toMutableList())
        }
    }

    fun copy() = PastMaxAux(tsZero?.copy(),
    ...
    fun update1(interval: Interval, nts: TS, ntp: TP, factor: Double, p1: Proof): Pair<Proof, PastMaxAux> {
        val copy = copy()
