...
class PastMinAux(
    ...
    private fun evalPMinaux(tp: TP, factor: Double): Proof {
        ...
        return if (correct >= factor) {
            ...
            VPastMinPrev(tp, vAlphasIn.first().second.at(), correct, vAlphasIn.map { it.second }.toMutableList())
        }
    }

    fun copy() = PastMinAux(tsZero?.copy(),
    ...
    fun update1(interval: Interval, nts: TS, ntp: TP, factor: Double, p1: Proof): Pair<Proof, PastMinAux> {
        val copy = copy()
