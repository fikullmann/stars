 ...
 * @param vAlphaBetasIn stores VProofs of type S⁻ where alpha is violated and beta is violated starting from that point until the end of the interval
 ...
 * @param vAlphasBetasOut stores all violations of alpha and beta outside the interval (are shifted into other violation proofs as needed)
 */
class Saux(
    ...
    private var vAlphasBetasOut: MutableList<Triple<TS, ViolationProof?, ViolationProof?>> = mutableListOf(),
): TAux() {
    ...
    private fun evalSaux(tp: TP): Proof {
        ...
        return p.reduce { acc, proof ->
            if (acc.size() <= proof.size()) acc else proof
        }
    }

    /**
     * update sBetaAlphasIn, sBetaAlphasOut, vAlphasOut, vAlphasBetasOut according to proof at current TS/TP
     */
    private fun addSubps(ts: TS, p1: Proof, p2: Proof) {
        when {
            ...
            p1 is ViolationProof && p2 is SatProof -> {
                ...
                sBetaAlphasOut.add(ts to SatSince(p2))
                // keep the smallest proofs in vAlphasOut, add p1 (will be removed next time if it is bigger than other proofs)
    ...
    /**
     * shifts the interval
     * and the proofs around in the different proof lists according to the current TS/TP
     */
    private fun shiftSaux(interval: BoundedInterval, iStart: Double, nts: TS, ntp: TP) {
        ...
        vAlphaBetasIn.forEachIndexed { i, _ ->
            vAlphaBetasIn[i].second.tp = ntp
        }
