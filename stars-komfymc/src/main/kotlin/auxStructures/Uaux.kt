...
class Uaux(
    private var sAlphasBeta: MutableList<MutableList<Pair<TS, SatUntil>>> = mutableListOf(mutableListOf()),
    private var sAlphasSuffix: MutableList<Pair<TS, SatProof>> = mutableListOf(),
    private var vBetasAlpha: MutableList<MutableList<Pair<TS, VUntil>>> = mutableListOf(mutableListOf()),
    ...
    private fun shiftUaux(interval: BoundedInterval, nts: TS, ntp: TP, end: Double? = null) {
        ...
        tsTps.forEach { (ts, tp) ->
            ...
            if (optimalProofsLen == optimalProofs.size) {
                ...
                if (vBAProof != null) {
                    if (vBAProof.at() == tsTpIn.firstOrNull()?.second) {
                        vBAProof.tp = tp
                ...
                val minProof = proofs.reduce { acc, newProof ->
    ...
    private fun adjustUaux(interval: BoundedInterval, nts: TS, ntp: TP) {
        ...
        vBetasAlpha.forEachIndexed { i, proofList ->
            vBetasAlpha[i] = proofList.dropWhile { (ts, proof) -> ts < firstTs + interval.startVal || (proof.at() < firstTp) }.toMutableList()
        }
        ...
        if (vBetasAlpha.isEmpty()) { vBetasAlpha.add(mutableListOf()) }
        // tstpin and out
        ...
        sAlphasBeta.first().let { frontAlphasBeta ->
            if (frontAlphasBeta.isNotEmpty()) {
                ...
                sAlphasBeta[0] = frontAlphasBeta.filter { (_, satProof) -> evalTp != satProof.at() || satProof.alphas.isNotEmpty() }
                    .map { (ts, satProof) ->
                        if (evalTp == satProof.at()) { satProof.alphas.removeFirst() }
                        ts to satProof
                    }.toMutableList()
            }
        }
        sAlphasBeta.forEachIndexed { i, proofList ->
            sAlphasBeta[i] = proofList.dropWhile { (_, p) ->
                tsTpOf(p.beta.at()) < (firstTs + interval.startVal)
            }.toMutableList()
        }
        sAlphasBeta = sAlphasBeta.dropWhile { it.isEmpty() }.toMutableList()
        if (sAlphasBeta.isEmpty()) { sAlphasBeta.add(mutableListOf()) }
        ...
        val shiftToOut: List<Pair<TS, Proof>> = vAlphasIn.filter { (ts, _) -> ts < firstTs + interval.startVal && ts >= firstTs}
        ...
        // vBetasIn
        vBetasSuffixIn = vBetasSuffixIn.dropWhile { (_, vp) ->
            when (tsTpIn.firstOrNull()) {
                null -> when (tsTpOut.firstOrNull()) {
                    null -> vp.at() <= ntp
    ...
    private fun dropUauxTs(a: Double, firstTs: TS) {
        vBetasAlpha.forEachIndexed { i, curBetasAlpha ->
            vBetasAlpha[i] = curBetasAlpha.fold(mutableListOf()) { acc, (vts, vp) ->
                // check if earliest timepoint in vProof is outside the bound (firstTs + a)
                // if out then remove the earliest part of the proof
                do {
                    var isOut = tsTpIn.find { (_, tp) -> tp == etp(vp) }?.let { (ts, _) -> ts < (firstTs + a) } ?: true
    ...
    fun copy() = Uaux(
        sAlphasBeta.map { it.map { inside -> inside.copy() }.toMutableList() }.toMutableList(),
        sAlphasSuffix.map { it.copy() }.toMutableList(),
        vBetasAlpha.map { it.map { inside -> inside.copy() }.toMutableList() }.toMutableList(),
    ...
    fun update1(interval: Interval, nts: TS, ntp: TP, p1: Proof, p2: Proof): Pair<MutableList<Proof>, Uaux> {
        val copy = copy()
