...
class CarlaTest() {
  ...
  val changedLaneDSL = formula { v: Ref<Vehicle> ->
    binding(term(v) { v -> v.lane }) { l ->
      eventually {
        pred(v) { v -> l.with(v) != v.lane }
      }
    }
  }
  ...
  val soBetweenDSL = formula { ego: Ref<Vehicle>, v1: Ref<Vehicle> ->
    exists { v2: Ref<Vehicle> ->
      (pred(ego, v2) { ego, v2 -> ego.id != v2.id } or
              pred(v1, v2) { v1, v2 -> v1.id != v2.id }) and
...
/*
  val followsDSL = formula { ego: Ref<Vehicle> ->
    exists { v1: Ref<Vehicle> ->
      eventually {
        globally(0.0 to 30.0) {
          behindDSL.holds(ego, v1)
        } and eventually(30.0 to 31.0) { tt() }
      }
    }
  }*/
  ...
  val rightOfDSL = formula { v1: Ref<Vehicle>, v2: Ref<Vehicle> ->
    besidesDSL.holds(v1, v2) and
            pred(v1, v2) { v1, v2 -> abs(v1.lane.laneId) > abs(v2.lane.laneId) }
  }
  ...
  val hasRelevantRedLightDSL = formula { v: Ref<Vehicle> ->
    eventually {
      hasRedLightDSL.holds(v) and isAtEndOfRoadDSL.holds(v)
    }
  }

  val didNotCrossRedLightDSL = formula { v: Ref<Vehicle> ->
    globally {
      hasRelevantRedLightDSL.holds(v) impl binding(term(v) { v -> v.lane.road }) { r ->
        next {
          pred(v) { v -> r.with(v) == v.lane.road }
        }
      }
    }
  }
  ...
  val hasStopSignDSL = formula { v: Ref<Vehicle> ->
    eventually { pred(v) { it.lane.hasStopSign } }
  }

  val hasYieldSignDSL = formula { v: Ref<Vehicle> ->
    eventually { pred(v) { it.lane.hasYieldSign } }
  }

  val mustYieldDSL = formula { v1: Ref<Vehicle> ->
    exists { v2: Ref<Vehicle> ->
      eventually {
        pred(v1, v2) { v1, v2 -> v1.lane.yieldLanes.any { it.lane == v2.lane } }
      }
    }
  }
